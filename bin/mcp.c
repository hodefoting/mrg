...
should listen for changes to file system, and based on these issue commands
...
static void enable_audio  (MCP *mcp);
static void disable_audio (MCP *mcp);
...
static void validate_client (MCP *mcp, const char *client_name)
{
  MrgList *l;
  for (l = mcp->clients; l; l = l->next)
  {
...

  {
    ...
    mrg_list_append (&mcp->clients, client);
  }
}

static void mcp_monitor_dir (MCP *mcp)
{
  MrgList *l;
again:
  for (l = mcp->clients; l; l = l->next)
  {
    ...
    {
      free (client);
      mrg_list_remove (&mcp->clients, client);
      goto again;
    }
  }

  DIR *dir = opendir (mcp->fbdir);
  struct dirent *ent;
  
  while ((ent = readdir (dir)))
  {
    if (ent->d_name[0]!='.')
      validate_client (mcp, ent->d_name);
  }
  closedir (dir);
}
...
static void monitor_audio (MCP *mcp, uint8_t *buf, int frames)
{
  MrgList *l;
...
void audio_callback (void *data, uint8_t *buf, int bytes)
{
  ...
  monitor_audio (mcp, target_buf, bytes / 2);
...
static void init_env (MCP *mcp)
{
  char buf[512];
  if (mcp->fbdir)
    return;
  mcp->fbdir = "/tmp/mrg";
  mcp->fbdir = getenv ("MMM_PATH");
  sprintf (buf, "mkdir %s &> /dev/null", mcp->fbdir);
  system (buf);
}

MCP *mcp_new (void)
{
  MCP *mcp = calloc (sizeof (mcp), 1);
  mcp->dev_audio = fopen ("/dev/audio", "w");
  return mcp;
}

void mcp_destroy (MCP *mcp)
{
  free (mcp);
}

static int mcp_idle_check (Mrg *mrg, void *data)
{
  MCP *mcp = data;
  MrgList *l;
  static int i = 0;
...
#if !MRG_SDL
  {
    ...
    memset (target_buf, 0, sizeof target_buf);
    monitor_audio (mcp, (void*)target_buf, bytes);
    for (i = 0; i < bytes; i++)
    {
    ...
    // XXX: convert to ulaw
    fwrite (temp2, bytes, 1, mcp->dev_audio);
...
int mcp_main (int argc, char **argv)
{
  ...
  if (!getenv ("MMM_PATH"))
  {
    fprintf (stderr, "mcp error MMM_PATH not set\n");
    return -1;
  }
  return 0;

  mrg = mrg_new (16, 16, NULL);
  mrg_set_title (mrg, "mcp");
  mcp = mcp_new ();
  mcp->mrg = mrg;
  mrg_set_ui (mrg, render_ui, mcp);

  init_env (mcp);
  mrg_add_idle (mrg, mcp_idle_check, mcp);
  mrg_main (mrg);
  mrg_destroy (mrg);
